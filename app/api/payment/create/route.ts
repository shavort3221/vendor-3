...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!orderId || !items || !buyerDetails) {
      return NextResponse.json({ error: "Missing required fields: orderId, items, buyerDetails" }, { status: 400 })
    }
    ...
    // Validate amount
    if (totalAmount < 1) {
      return NextResponse.json({ error: "Total amount must be at least ₹1" }, { status: 400 })
    }

    // Create payment purpose
    const purpose = groupOrderId ? `Group Order Payment - ${groupOrderId}` : `Order Payment - ${orderId}`
    ...
    if (!validation.valid) {
      return NextResponse.json({ error: "Invalid payment data", details: validation.errors }, { status: 400 })
    }
    ...
    if (!paymentResponse.success) {
      return NextResponse.json({ error: paymentResponse.error || "Failed to create payment request" }, { status: 500 })
    }
